...
<Accordion collapsible>
	...
	>
		{#snippet control()}
			<header class="h2 font-bold mb-4 mt-3">Ports</header>
		{/snippet}
		{#snippet panel()}
			<div>
				<section class="p-4 text-lg">
					<div class="grid grid-cols-6 row-auto justify-items-center gap-y-2 text-sm md:text-base">
						...
						<div class="font-bold col-span-2 md:col-span-1 text-xs md:text-base">
							Configuration/MSP
						</div>
						...
						{#each serial as port, i (i)}
							...
							<div class="col-span-2 md:col-span-1">
								{#if msp.some((msp) => port.function.includes(msp))}
									<div class="bg-primary-500 w-14 h-6 rounded-full relative">
										<div
											class="bg-white w-5 aspect-square rounded-full absolute top-0.5 right-0.5"
										></div>
									</div>
								{:else}
									<div class="bg-surface-500 w-14 h-6 rounded-full relative">
										<div
											class="bg-white w-5 aspect-square rounded-full absolute top-0.5 left-0.5"
										></div>
									</div>
								{/if}
							</div>
							...
							<div class="col-span-2 md:col-span-1">
								{#if telemetryNames.some((telemetry) => port.function.includes(telemetry))}
									<div>
										...
										{Array.isArray(port.function)
											? port.function.filter((peripheral) => telemetryNames.includes(peripheral))
											: []}
								...
								{:else}
									<div class="text-surface-300">
										Disabled <span class="text-surface-400"
											>{port.telemetry === 0 ? "Auto" : port.telemetry}</span
										>
									</div>
								{/if}
							</div>
							<div class="col-span-2 md:col-span-1">
								{#if sensorNames.some((sensor) => port.function.includes(sensor))}
									<div>
										{Array.isArray(port.function)
											? port.function.filter((peripheral) => sensorNames.includes(peripheral))
											: []}
										<!-- if the function is "GPS", show gps value instead of telemetry value -->
										<span class="text-surface-400">
											{port.function.includes("GPS")
												? port.gps
												: port.telemetry === 0
													? "Auto"
													: port.telemetry}
							...
							<div class="col-span-2 md:col-span-1">
								{#if peripheralNames.some((peripheral) => port.function.includes(peripheral))}
									<div>
										{Array.isArray(port.function)
											? port.function.filter((peripheral) => peripheralNames.includes(peripheral))
											: []}
										<span class="text-surface-400">
											{port.telemetry === 0 ? "Auto" : port.telemetry}
										</span>
